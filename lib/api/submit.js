...
module.exports = {
  exec(context, { settings, form }) {
    return validateForm({ settings, form })
      .then(() => validateRecaptcha(context, { settings, form }))
...
const validateRecaptcha = (context, { settings, form }) => {
  return new Promise((resolve, reject) => {
    if (!settings.fields.recaptcha) resolve();

    return axios
      .post(
        `https://www.google.com/recaptcha/api/siteverify?secret=${process.env.RECAPTCHA_SECRET}&response=${form.recaptcha}`
      )
      .then(response => {
        if (!response.data.success) reject(response.data['error-codes']);
